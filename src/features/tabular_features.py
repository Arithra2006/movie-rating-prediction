...
def director_success_rate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    director_avg = df.groupby(dir_col)['vote_average'].mean().reset_index()
...
def actor_success_rate(df: pd.DataFrame) -> pd.DataFrame:
    """Calculate average rating per lead actor."""
    ...
    # Use Star1 as lead actor
    lead_col = actor_cols[0]
    actor_avg = df.groupby(lead_col)['vote_average'].mean().reset_index()
...
def genre_popularity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    genre_avg = df.groupby(genre_col)['vote_average'].mean().reset_index()
...
def budget_revenue_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'budget' in df.columns and 'revenue' in df.columns:
        ...
        df['log_popularity'] = np.log1p(df['popularity']) if 'popularity' in df.columns else 0
...
if __name__ == "__main__":
    ...
    print(df[['title', 'vote_average', 'director_success_rate', 'log_budget']].head(10))